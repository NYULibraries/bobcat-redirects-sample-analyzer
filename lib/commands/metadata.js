...
import { BobCatClassicSampleAnalyzer } from "../classes/BobCatClassicSampleAnalyzer.js";
...
function metadataReportCommand( sampleDirectory ) {
    ...
    const sampleAnalyzers = [
        new BobCatClassicSampleAnalyzer(),
    ...
    Object.keys( index ).sort().forEach( testCaseUrl => {
        ...
        sampleAnalyzers.forEach( sampleAnalyzer => {
            ...
            // Some index entries record failures: no sample file, only screenshots.
            if ( ! sampleFile ) {
                return;
            }
        ...
        // Samples at this time can appear in index.json even if they were
        // unsuccessful.  Don't write a metadata report entry if none of the
        // samplers produced a sample HTML file.
        if ( Object.keys( metadataReportEntry.title ).length === 0 ) {
            return;
        }
